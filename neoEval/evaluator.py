...
class Evaluator(object):
    ...
    def _five(self, cards):
        ...
        """
        # if flush
        if cards[0].bin & cards[1].bin & cards[2].bin & cards[3].bin & cards[4].bin & 0xF000:
            handOR = (cards[0].bin | cards[1].bin | cards[2].bin | cards[3].bin | cards[4].bin) >> 16
    ...
    def hand_equity(self, board, hands):
        """
        Gives a sumamry of the hand with ranks as time proceeds.
        ...
        for i in range(len(stages)):
            ...
            for player, hand in enumerate(hands):
                ...
                if rank == best_rank:
                    winners.append(player)
                    winners_log[i].append(player)
                    best_rank = rank
                elif rank < best_rank:
                    winners = [player]
                    winners_log[i] = [player]
                    best_rank = rank
